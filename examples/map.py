...
if __name__ == "__main__":
    ...
    img = (np.random.random((1000, 1000, 3)) * 255).astype(np.uint8)
    map = Map(img, auto_reload=True)
    def tick():
        # return
        map.update((np.random.random((1000, 1000, 3)) * 255).astype(np.uint8))
    win = MeshViewWindow(add_floorgrid=False, orthographic=False, target_fps=60, tick_func=tick, enable_control=True)

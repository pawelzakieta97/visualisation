...
class Cone(Mesh):
    def __init__(self, radius=1, segments: int = 16, height: float = 1,
                 center_point=False, single_top=True,
                 separate_base=True):

        base = Circle(radius=radius, vert_count=segments, center_point=False)
        ...
        side_triangles[:, 1:] = np.lib.stride_tricks.sliding_window_view(cycled_base_indices[::-1], 2)
        if single_top:
        ...
        if separate_base:
            base = Circle(radius=radius, vert_count=segments, center_point=center_point)
            base.transform_mesh(get_rotation_matrix_x(np.pi/2))
            ...
            super().__init__(side_vertices, np.vstack((base.triangle_indices, side_triangles)))
